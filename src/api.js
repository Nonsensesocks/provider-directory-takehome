 ...
 */
 ...
 const makePartialRepresentation = ({id, name, title, designation, avatarUrl, bio, availability, location}) => ({
   id, name, title, designation, avatarUrl, bio, availability, location
 });
 
 // Fetch Locations, this is to enable locations in the main dropdown that have providers
 export const fetchProviderLocation = () => {
   return new Promise(
     (resolve) => setTimeout(
       () => {
         ...
         const uniqLocations = removeDuplicates(locations)
         resolve(uniqLocations)
       },
       REQUEST_RESPONSE_TIME
     )
   )
